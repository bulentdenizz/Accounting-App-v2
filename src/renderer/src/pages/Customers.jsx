...
export default function Customers() {
  ...
  const fetchData = async () => {
    try {
      ...
      setCustomers(dbData.filter(item => item.type === 'Customer'));
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Strict Regex validation (e.g. 0 555 555 5555)
  ...
  const handleDelete = async (id, title) => {
    const message = t('msg_confirm_delete').replace('{{name}}', title);
  ...
  const handleEdit = (customer) => {
    ...
    setFormData({ title: customer.title, phone: customer.phone, type: 'Customer', address: customer.address || '' });
    setIsModalOpen(true);
  };
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            ...
            <span className="text-xs bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-2 py-1 rounded-full border border-slate-200 dark:border-slate-700">
              {t('record_total').replace('{{count}}', customers.length)}
            </span>
          </h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">{t('customers_desc')}</p>
        </div>
             ...
             <table className="fin-table">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  {filteredCustomers.length === 0 && (
                     <tr><td colSpan="4" className="p-12 text-center text-slate-400 italic">{t('msg_no_customer')}</td></tr>
                  )}

                  {filteredCustomers.map(m => (
           ...
           <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
               <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/30">
                  <h3 className="font-bold text-slate-900 dark:text-white text-lg">
                     {editingId ? t('modal_edit_customer') : t('modal_new_customer')}
                  </h3>
